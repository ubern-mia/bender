...
Training artifacts (model checkpoints, TensorBoard logs) are written to a versioned subfolder under `results/` and are excluded from git. Plot images and other reference outputs live alongside the script in each experiment folder and are tracked in git:

```
dermamnist_v1_initial/          (SGD, lr=5e-6, momentum=0.5)
dermamnist_v2_momentum0p9/      (SGD, lr=5e-6, momentum=0.9)
dermamnist_v3_lr0p005_val_patience/ (SGD, lr=0.005, momentum=0.9 + early stopping)
dermamnist_v4_adam_TB/          (Adam, lr=0.005 + TensorBoard)
dermamnist_v5_deeper_network/   (Adam + 6-layer CNN)
dermamnist_v6_even_deeper_network/ (Adam + 8-layer CNN)
dermamnist_v7_with_augm/        (Adam + 8-layer CNN + data augmentation)
```
...
| `per_class_metrics.png` | Per-class precision/recall breakdown (v1) |
| `train_val_TB_plots.png` | TensorBoard screenshot of training + validation curves (v4+) |
| `test_accuracy_vN.png` | Classification report on the test set (v4+) |
| `model_summary.txt` | Layer-by-layer parameter count via **torchinfo** |
| `model_graph.png` | Computation graph image via **torchview** |

> `best_model.pt`, `model.onnx`, checkpoints, and TensorBoard event files land in `results/vN/` and are excluded from git.
...
![validation accuracy after learning rate change](dermamnist_v3_lr0p005_val_patience/val_acc.png)
