...
Dependencies are managed with [uv](https://github.com/astral-sh/uv). The **[Makefile](../Makefile) lives at the repo root** alongside the shared `.venv/`. Run all commands from there:

```bash
make setup      # create .venv using uv
...
make run-v1     # run a specific version (run-v1 through run-v7)
```
...
All training artifacts (model checkpoints, loss/accuracy plots, TensorBoard logs) are written to a versioned subfolder under `results/` and are excluded from git:

```
results/
  v1/   ← dermamnist_v1_initial          (SGD, lr=5e-6, momentum=0.5)
  v2/   ← dermamnist_v2_momentum0p9      (SGD, lr=5e-6, momentum=0.9)
  v3/   ← dermamnist_v3_lr0p005_patience (SGD, lr=0.005, momentum=0.9 + early stopping)
  v4/   ← dermamnist_v4_adam_TB          (Adam, lr=0.005 + TensorBoard)
  v5/   ← dermamnist_v5_deeper_network   (Adam + 6-layer CNN)
  v6/   ← dermamnist_v6_even_deeper      (Adam + 8-layer CNN)
  v7/   ← dermamnist_v7_with_augm        (Adam + 8-layer CNN + data augmentation)
```
...
Key takeaways:
- The single biggest jump is **v2 → v3**: correcting the learning rate (×1000) adds ~10 pp of test accuracy.
- Switching to Adam (v4) adds ~1 pp while making training less sensitive to hyperparameters.
- Simply adding more layers (v5, v6) hurts — the model overfits without regularization.
- Data augmentation (v7) recovers the overfit and yields the best result, beating the MedMNIST leaderboard at time of writing.

--------------------

## Model visualization

Each script exposes a `VISUALIZE_MODE` flag in its `__main__` block. Set `TRAINING_MODE = False` and `VISUALIZE_MODE = True`, then run the script. Three artifacts are produced in `results/vN/`:

### torchinfo — layer-by-layer summary

`model_summary.txt` contains a table of every layer with its output shape, parameter count, and memory usage, generated by [`torchinfo`](https://github.com/TylerYep/torchinfo).

### torchview — computation graph

`model_graph.png` is a rendered dataflow graph of the network (requires [Graphviz](https://graphviz.org) to be installed), generated by [`torchview`](https://github.com/mert-kurttutan/torchview).

### netron — interactive ONNX viewer

`model.onnx` is an ONNX export of the untrained architecture. Open it with:

```bash
netron results/vN/model.onnx
```

[`netron`](https://netron.app) launches a local browser tab where you can click through every layer, inspect tensor shapes, and explore the full graph interactively. It also works directly at [netron.app](https://netron.app) by dragging the file in.
...
![training loss after momemtum change](results/v2/train_loss.png)
...
![validation accuracy after momemtum change](results/v2/val_acc.png)
...
![training loss after change in learning rate](results/v3/train_loss.png)
...
![validation accuracy after learning rate change](results/v3/val_acc.png)
...
![Training and Validation losses and accuracies](results/v4/train_val_TB_plots.png)
...
![Test accuracy and classification report](results/v4/test_accuracy_v4.png)
...
![Training and Validation losses and accuracies](results/v5/training_val_v4_v5_comparison.png)
...
![Test accuracy and classification report](results/v5/test_accuracy_v5.png)
...
![Training and Validation losses and accuracies](results/v6/training_val_comparison_v4_v5_v6.png)
...
![Test accuracy and classification report](results/v6/test_accuracy_v6.png)
...
![Training and Validation losses and accuracies](results/v7/training_val_v4_7.png)
